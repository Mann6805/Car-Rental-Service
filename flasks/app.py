...
def update_availability():
    ...
    # Query to select vehicles where the booking end date is before today
    select_query = """
        SELECT V.VEHICLE_ID
        FROM BOOKING B
    ...
    print(vehicles_to_update)

    for vehicle_id_tuple in vehicles_to_update:
        vehicle_id = vehicle_id_tuple[0]
        update_query = """
            UPDATE VEHICLE
            SET AVAILABLE = TRUE
            WHERE VEHICLE_ID = %s
